...
public class ClienteService {
    ...
    public List<Cliente> getAllClientesInactivos() throws Exception {
        Session session = HibernateUtils.getSessionFactory().getCurrentSession();
        Transaction tx = session.beginTransaction();
        List<Cliente> clientes = null;
        try {
            clientes = new ClienteBO().getAllClientesInactivos();
            tx.commit();
        } catch (Exception ex) {
            tx.rollback();
            throw new Exception(ex);
    ...
    public Cliente updateClientesConAbonos(Cliente cliente, List<Abono> abonos) throws Exception {
        Session session = HibernateUtils.getSessionFactory().getCurrentSession();
        Transaction tx = session.beginTransaction();
        DireccionBO direccionBO = new DireccionBO();
        Direccion direccion = cliente.getDireccion();
    ...
    public List<Cliente> getAllClientesHabilitadosByRubro(Rubro rubro) throws Exception {
        Session session = HibernateUtils.getSessionFactory().getCurrentSession();
        Transaction tx = session.beginTransaction();
        List<Cliente> clientes = null;
        try{
            clientes = new ClienteBO().getAllClientesHabilitadosByRubro(rubro);
            tx.commit();
        }catch(Exception ex){
            tx.rollback();
            throw new Exception(ex);
