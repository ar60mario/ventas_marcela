...
public class ConfiguracionService {
    ...
    public Configuracion getConfiguracionByTitular(TitularCuit titular) throws Exception {
        Session session = HibernateUtils.getSessionFactory().getCurrentSession();
        Transaction tx = session.beginTransaction();
        Configuracion conf = null;
        try {
            conf = new ConfiguracionBO().getConfiguracionByTitular(titular);
            tx.commit();
        } catch (Exception ex) {
            tx.rollback();
            throw new Exception(ex);
    ...
    public Configuracion saveConfiguracion(Configuracion configuracion) throws Exception {
        ...
        Configuracion conf = null;
        try {
            conf = new ConfiguracionBO().saveConfiguracion(configuracion);
            tx.commit();
        } catch (Exception ex) {
            tx.rollback();
            throw new Exception(ex);
