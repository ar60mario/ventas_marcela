...
public class FacturaService {
    ...
    public void saveFacturaCompleta(Factura factura, List<RenglonFactura> renglones) throws Exception{
        Session session = HibernateUtils.getSessionFactory().getCurrentSession();
        Transaction tx = session.beginTransaction();
        try {
            factura = new FacturaBO().saveFactura(factura);
            ...
            new RenglonFacturaBO().saveRenglonesFactura(renglones);
            tx.commit();
        } catch (Exception ex) {
            tx.rollback();
            throw new Exception(ex);
    ...
    public List<Factura> getFacturasEntreFechasAndRubro(Date fd, Date fa, Rubro ru) throws Exception{
        ...
        List<Factura> facturas = null;
        try {
            facturas = new FacturaBO().getFacturasEntreFechasAndRubro(fd, fa, ru);
            tx.commit();
        } catch (Exception ex) {
            tx.rollback();
            throw new Exception(ex);
