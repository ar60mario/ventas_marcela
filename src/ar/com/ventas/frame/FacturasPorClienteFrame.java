...
public class FacturasPorClienteFrame extends javax.swing.JFrame {
    ...
    private void verFactura() {
        ...
        if (row < 0) {
            JOptionPane.showMessageDialog(this, "SELECCIONE LA FACTURA/NOTA DE CREDITO QUE QUIERE VER");
            return;
        }
        Factura fa = facturas.get(row);
        VerFacturaFrame vff = new VerFacturaFrame(fa);
        vff.setVisible(true);
        this.dispose();
    }
    ...
    private void llenarTabla(int row) {
        ...
        facturas = null;
        try {
            facturas = new FacturaService().getFcAndNcPorCliente(cliente);
        } catch (Exception ex) {
        ...
        if (facturas != null && !facturas.isEmpty()) {
            ...
            for (Factura f : facturas) {
                ...
                if (f.getTipoDoc().equals(11)) {
                    o[1] = "FC";
                } else {
                    o[1] = "NC";
                }
                o[2] = f.getNumero();
                o[3] = df.format(f.getImporte());
                tbl.addRow(o);
            }
            tabla.setModel(tbl);
        }
